...
class LiveScanModel extends Dbh {
  ...
  public function insertLiveScan($dataArr) {
    $db = $this->db();
    $sql = "INSERT INTO live (liveInitTS, liveInitLat, liveInitLon, liveDirection, liveVesselID, liveName, liveLength, liveWidth, liveDraft, liveCallSign, liveSpeed, liveCourse, liveDest, liveScanLocal) VALUES (:liveInitTS, :liveInitLat, :liveInitLon, :liveDirection, :liveVesselID, :liveName, :liveLength, :liveWidth, :liveDraft, :liveCallSign, :liveSpeed, :liveCourse, :liveDest, :liveScanLocal)";
     $ret = $db->prepare($sql);
     $ret->execute($dataArr);
